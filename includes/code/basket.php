...
$sql="select date_format(to_time,'%W, %d %M %Y') as to_time1,date_format(to_time,'%W, %d %b %Y') as to_time2,date_format(to_time,'%a %b %Y') as to_time3,to_time,match6,now() as current_date1,vision_price,vision_winning_amount from naira_lotto where ( now() between from_time and to_time )";
...
if($rs && !$rs->EOF){
  ...
  $naira[]=stripslashes($rs->fields["to_time3"]);
} else {
  if($rs) $rs->close();
  header("location: ".$_DIR["site"]["url"]."naira-lotto".$atend);
  exit();
}
if($rs) $rs->close();
...
if($_SESSION["MY_VISION_BASKET"]) $my_basket = unserialize($_SESSION["MY_VISION_BASKET"]);

if($my_basket['vision']) $vision_number=$my_basket['vision'];
else $vision_number=get_vision_number();

//if naira delete play slip link clicked
if($_GET["act"]=="rmnairapslip" && $_GET["rowid"]) {
	print_r($my_basket);
	$new_basket=array();
	...
	$_SESSION["MY_VISION_BASKET"]=serialize($new_basket);
...
if(strtoupper($_SERVER['REQUEST_METHOD'])=="POST" && $_POST["hidAction"]=="proceed-button-click") {
	if(isset($my_basket) && count($my_basket)>0) {
		...
		header("location: ".$_DIR["site"]["url"]."check-number".$atend);
		exit();
	}
}
...
if($_GET["act"]=="editnairapslip" && $_GET["rowid"]) {
	if($_SESSION["MY_VISION_BASKET"]) {
	  $len=$_GET["rowid"]-1;
	  for($i=1,$k=0;$i<=42;$i+=6,$k++)
	   	for($j=$i,$p=0;$j<=($i+5);$j++,$p++) {
