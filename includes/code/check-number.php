...
if($rs && !$rs->EOF){
  ...
  $naira[]=stripslashes($rs->fields["naira_id"]);
} else {
  if($rs) $rs->close();
...
if($_GET["act"]=="savenumber") {
	...
	$_CONN->Execute($sql);
	$flag=false;
	$sql="insert into saved_entry values (NULL,'".$naira[6]."','N','".$_SESSION['login']['userid']."','".$_WEEK_DAY[$my_basket["draws"]]."','".$my_basket['weeks']."')";
