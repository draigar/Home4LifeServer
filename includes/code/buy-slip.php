...
if($total_amount>get_balance()) {
  header("location: ".$_DIR["site"]["url"]."transfer-amount".$atend."pvs".$_DELIM."check-number".$baratend);
  exit();
}
...
if($_POST["hidAction"]=="buynow" && $_SESSION["MY_VISION_BASKET"]) {
	...
	$result=save_entry("naira",$naira[7],($_POST["INDEX"]-1),$price,$naira[5]);
	if($result[0]) {
		...
		if(get_basket_count()==$_POST["INDEX"]) {
			...
			header("location: ".$_DIR["site"]["url"]."summary".$atend."idx".$_DELIM."i".md5($result[2]).$baratend);
            exit();
		}
	} else {
		$error=true;
