...
if($_SERVER['REQUEST_METHOD'] == "POST") { 
	...
	if(trim($_SESSION['login']['username'])==trim($_POST["txtEmail"])) { 
		...
		$sql="select user_id from login_info where user_id!='".$_SESSION['login']['userid']."' and username='".trim($_POST["txtEmail"])."'";
	...
	if(!$error){
		...
		if($_CONN->Execute($sql)) { 
			...
			$sql="insert into trans_fund_to values (NULL,'".$intID."','".$_SESSION['login']['userid']."')";
