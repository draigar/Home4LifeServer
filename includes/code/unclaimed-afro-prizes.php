...
if($rs) $rs->close();
$sql="select c.lotto_numbers,e.entry_id,e.vision_numbers,l.match0,l.match1,l.match2,l.match3,l.match4,l.match5,l.match6,l.match7,l.draw_number,l.draw_date,
...
where l.status='D' and t.cancel='N' and m.claimed_id is null and date_format(date_add(l.draw_date,interval ".$ulimit." DAY),'%Y-%m-%d')>=curdate() group by c.ne_id 
having ( 
	( (matchx1+matchx2+matchx3+matchx4+matchx5+matchx6+matchx7)>=5 ) 
	OR ( l.vision_winning_amount>0 and vismatch=1 ) 
