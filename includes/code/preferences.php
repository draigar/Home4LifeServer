...
$suc = false;
	$_sql_sp= "DELETE FROM preferences WHERE user_id=".$_SESSION['login']['userid']." AND user_type='U'";
	$_CONN->Execute($_sql_sp);
	
	for($i=0;$i<count($_POST['chkRollover']);$i++){
		 $_sqlQue1 ="INSERT INTO preferences(user_id,newsletter_id,user_type) VALUES('".$_SESSION['login']['userid']."','".$_POST['chkRollover'][$i]."','U')"; 
