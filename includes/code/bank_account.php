...
if(!empty($_GET['id']) && $_GET['act']=="edit"){ 
	$_sql = "SELECT * FROM user_bank_info WHERE user_id=".$_SESSION['login']['userid']." and ubank_id=".$_GET['id'];
	$rs = $_CONN->Execute($_sql);
	if ($rs->EOF) { 
			$_MSG[0]="Invalid Bank ID, Please try with valid id. ";
			$error = 1;
    }
	elseif($_SERVER['REQUEST_METHOD'] != "POST") 
	{	    
	   
	   		$_POST["txtACCName"] 		  	= stripslashes($rs->fields["acnt_name"]);
			$_POST["txtACCNo"]        	= stripslashes($rs->fields["acnt_no"]);
			$_POST["cmbAType"] 		  	= stripslashes($rs->fields["acnt_type"]);
...
if($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['Input']=='Update'){	
	 ...
	 $_sql = "Update
		    ...
		    acnt_name             	         =      '".trim($_POST['txtACCName'])."',
		    acnt_no                        =       '".trim($_POST['txtACCNo'])."',
            acnt_type                       =         '".trim($_POST['cmbAType'])."',
            ...
            country_id                      =      '1', 
			approved='N'
			WHERE ubank_id= ".$_GET["id"];

			if($_CONN->Execute($_sql)){ 
			  $intID=$_CONN->Insert_ID();
			  
			 if($_POST["chkPrimary"]=="Y"){ 
			    $_sql = "update user_bank_info set cpramary='N' where user_id=".$_SESSION['login']['userid']." and ubank_id!=".$_GET["id"];
				$_CONN->Execute($_sql);
			 }
...
if($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['Input']=='Submit'){	
	...
	if($_POST['chkPrimary']==""){
	   $_POST['chkPrimary'] = "N";
		...
		if(empty($error)){
		  $_sql = "insert into user_bank_info(user_id,bank_name,address1,acnt_name,acnt_no,cpramary,
				         acnt_type,address2,city,state,lga,country_id,bank_swiftcode,bank_othercode,approved)
	     values('".$_SESSION['login']['userid']."',
	        '".trim($_POST['txtBankName'])."',
			'".trim($_POST['txtBankAdd'])."',
			'".trim($_POST['txtACCName'])."',
			'".trim($_POST['txtACCNo'])."',
			'".trim($_POST['chkPrimary'])."',
			'".trim($_POST['cmbAType'])."',
			...
			if($_CONN->Execute($_sql)){ 
			  $intID=$_CONN->Insert_ID();
			 if($_POST["chkPrimary"]="Y"){ 
			    $_sql = "update user_bank_info set cpramary='N' where user_id=".$_SESSION['login']['userid']." and ubank_id!=".$intID;
				  $_CONN->Execute($_sql);
			 }
	...
	$getAllIdsqry 			= $_DIR['inc']['queries']."getaccountids.sql";	//This will spcify the fiel which will give us only the ids of the country
	...
	$getDataByIdsqry 		= $_DIR['inc']['queries']."getaccountbyids.sql";	//This is the query file which will give us the country inforation by passing the ids got from just above string
