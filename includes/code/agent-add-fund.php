...
if($_SERVER['REQUEST_METHOD'] == "POST" && $_POST["Input1"]=="Submit1") {
...
if(!$error && $_SERVER['REQUEST_METHOD'] == "POST" && $_POST["Input1"]!="Submit1") {
	...
	if(!$error){ 
	   if($_POST["cmbMethod"]=="V") {
			...
			$balance=get_balance()+$_POST['txtAmount'];
	   		$sql="insert into transaction values(NULL,'".$identifier."',now(),0,'".$_POST['txtAmount']."','".$_SESSION['login']['userid']."','C','P','',0,0,'".trim($_POST["txtSerNo"])."','D','U','".$balance."')";
