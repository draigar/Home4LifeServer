<?PHP
	if(empty($_GET["srt"]))
		$_GET["srt"]=" desc ";
	 $_sql = "SELECT merchant.*,concat(fname,' ',lname) as name,cordinate_serial.serial_no,cordinate_serial.status as cord_status,
			cordinate_serial.user_id as c_user_id ,date_format(acc_create_date,'%d-%b-%Y')as acc_create_date
	        FROM merchant 
