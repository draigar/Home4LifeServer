<?PHP
	if(empty($_GET["srt"]))
		$_GET["srt"]=" desc ";
	$where = " where merchant.status in ('A','S') and terminal_agent.term_id is null ";
	...
	$_sql = "SELECT merchant.user_id FROM merchant left join terminal_agent on terminal_agent.agent_id = merchant.user_id
	".$where." group by merchant.user_id ORDER BY ".($_GET["oby"]!="" ? $_GET["oby"] : "merchant.user_id")." ".$_GET["srt"]; 
?>