<?PHP
if(empty($_GET["srt"]))
		$_GET["srt"]=" desc ";
		
   $orderByClause = ($_GET["oby"]!="" ? $_GET["oby"] : "n.nt_id")." ".$_GET["srt"];

   $_sql = "SELECT 

			n.nt_id,l.al_id,date_format(e.date_entered,'%d-%b-%Y %h:%i') as date_entered,n.ticket_no,

			date_format(e.date_entered,'%d%m%y') as date_entered2,e.method_id,e.term_id,count(c.ne_id)as entries,e.vision_numbers

		FROM 

			afro_entry_ticket n

			inner join afro_entry e on e.entry_id=n.entry_id 

			inner join afro_lotto l on l.afro_id=e.afro_id

			left join afro_entry_child c on c.nt_id=n.nt_id 

		WHERE 

			n.nt_id in (".$idsToDisplay.") group by n.nt_id

		ORDER BY 

			".$orderByClause;				

?>