...
if(!empty($_GET['aid']) && $_GET['act']=="res"){
	...
	if (!$error) { 
		$_sql="UPDATE naira_lotto SET status='A' WHERE naira_id=".$_GET['aid'];
		$_CONN->Execute($_sql);
		header("Location: ".$_DIR['site']['adminurl']."cancelled_naira".$atend."success".$_DELIM."4".$baratend);
		exit(); 
	}
...
//==============Code added by Jay=========================
	...
	$getAllIdsqry 			= $_DIR['admininc']['queries']."getcancelled_nairaids.sql";	//This will spcify the fiel which will give us only the ids of the country
	...
	$getDataByIdsqry 		= $_DIR['admininc']['queries']."getcancelled_nairabyids.sql";	//This is the query file which will give us the country inforation by passing the ids got from just above string
...
if ($_GET["success"]==4)
	$success = "The Naira Lotto has been successfully Resume.";
