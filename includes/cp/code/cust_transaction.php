...
if(!empty($_GET['aid']) && $_GET['act']=="sus") {
	$_sql="UPDATE transaction SET status='S' WHERE trans_id=".$_GET['aid'];
	if ($_CONN->Execute($_sql)) { 
		$success = "The Transaction has been suspended successfully.";
	}else{	
	     $_MSG[] = "";
		 $error = 1;
	}
...
$getAllIdsqry 			= $_DIR['admininc']['queries']."getcust_transactionids.sql";	//This will spcify the fiel which will give us only the ids of the country
...
$getDataByIdsqry 		= $_DIR['admininc']['queries']."getcust_transactionbyids.sql";	//This is the query file which will give us the country inforation by passing the ids got from just above string
