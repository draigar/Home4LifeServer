...
if($rs) $rs->close();
$sql="select c.lotto_numbers,e.entry_id,e.vision_numbers,l.match0,l.match1,l.match2,l.match3,l.match4,l.match5,l.match6,
l.match7,l.draw_number,l.draw_date,l.matchz3,l.matchz4,l.matchz5,l.matchz6,
...
where l.status='D' and m.claimed_id is null and date_format(date_add(l.draw_date,interval ".$ulimit." DAY),'%Y-%m-%d')>=curdate() group by c.ne_id   
having ( 
	( 
				( e.method_id='T' and (matchx1+matchx2+matchx3+matchx4+matchx5+matchx6+matchx7)>=3 )  
				or 
				( e.method_id!='T' and (matchx1+matchx2+matchx3+matchx4+matchx5+matchx6+matchx7)>3 ) 
			)
	OR 
	( l.vision_winning_amount>0 and vismatch=1 )
	) order by l.draw_date desc,l.afro_id";
$result=array(); 
$rs =$_CONN->Execute($sql);
$i=0;
...
while(!$rs->EOF){
	...
	if($rs->fields['matchp7']) { if($win_num[$i]) $win_num[$i].=","; $win_num[$i].=$rs->fields['matchp7']; $ki++; }
	if($rs->fields['method_id']=='T' && $ki<3) $win_num[$i]="";
	elseif($rs->fields['method_id']!='T' && $ki<4) $win_num[$i]="";
