...
if(!empty($_GET['aid']) && $_GET['act']=="del") //Delete Action
{
	
	 $_sql="DELETE FROM claimpoint
			   WHERE cpoint_id=".$_GET['aid'];
	if ($rs=$_CONN->Execute($_sql)) { 
		$success = "Claimpoint has been successfully deleted.";
		$rs->close();
	}else{	
	     $_MSG[] = "";
		 $error = 1;
...
//==============Code added by Jay=========================
	...
	$getAllIdsqry 			= $_DIR['admininc']['queries']."get_claimpointids.sql";	//This will spcify the fiel which will give us only the ids of the country
	...
	$getDataByIdsqry 		= $_DIR['admininc']['queries']."get_claimpointbyids.sql";	//This is the query file which will give us the country inforation by passing the ids got from just above string
...
if ($_GET["success"]==1)
	$success = "The Claimpoint has been successfully added.";
if ($_GET["success"]==2)
	$success = "The Claimpoint has been successfully updated.";
?>