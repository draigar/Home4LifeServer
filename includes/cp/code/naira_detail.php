...
if($_SERVER['REQUEST_METHOD']=="POST" && $_POST['Submit']=="Notify Result") {
	$sql="update naira_lotto set status='D',matchz3='".trim($_POST["txtMatch3"])."',matchz4='".trim($_POST["txtMatch4"])."',
	matchz5='".trim($_POST["txtMatch5"])."',good_cause='".trim($_POST["txtGoodCause"])."' where naira_id=".$_GET['id'];
	if($_CONN->Execute($sql)) {
		notify_draw_result("naira",$_GET["id"],6);
		header("Location: ".$_DIR['site']['adminurl']."naira_listing".$atend."success".$_DELIM."4".$baratend);
		exit();
	} else {
