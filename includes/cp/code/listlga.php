...
if(!empty($_GET['aid']) && $_GET['act']=="del") //Delete Action
{
	 ...
	 $_sql="DELETE FROM lga
			   WHERE lga_id=".$_GET['aid'];
	if ($rs=$_CONN->Execute($_sql)) { 
		$success = "The LGA has been successfully deleted.";
		$rs->close();
	}else{	
	     $_MSG[] = "";
		 $error = 1;
...
//==============Code added by Jay=========================
	...
	$getAllIdsqry 			= $_DIR['admininc']['queries']."getlgaids.sql";	//This will spcify the fiel which will give us only the ids of the country
	...
	$getDataByIdsqry 		= $_DIR['admininc']['queries']."getlgabyids.sql";	//This is the query file which will give us the country inforation by passing the ids got from just above string
...
if ($_GET["success"]==1)
	$success = "LGA has been successfully added.";
if ($_GET["success"]==2)
	$success = "LGA has been successfully updated.";
?>