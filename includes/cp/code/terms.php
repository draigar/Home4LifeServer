...
{
	$_sql = "SELECT terms_id, image FROM terms WHERE terms_id=".$_GET['nid'];
	...
	if ($rs->EOF) {
		...
		$terms_image = $rs->fields["image"];
	if ($rs)
		$rs->close();
	if (!$error) {
		if(!empty($terms_image) && file_exists($_DIR['inc']['terms_image'].$terms_image))
		{
				unlink($_DIR['inc']['terms_image'].$terms_image);
				unlink($_DIR['inc']['terms_image']."t".$terms_image);
		}
		$_sql = "delete from terms where terms_id=".$_GET['nid'];
		$_CONN->Execute($_sql);
		header("Location: ".$_DIR['site']['adminurl']."terms".$atend."suc".$_DELIM."3".$baratend);
		exit();
	}
}
if(empty($_POST['num']))
...
//==============Code added by Jay=========================
	...
	$getAllIdsqry 			= $_DIR['admininc']['queries']."gettermsids.sql";	//This will spcify the fiel which will give us only the ids of the country
	...
	$getDataByIdsqry 		= $_DIR['admininc']['queries']."gettermsbyids.sql";//This is the query file which will give us the country inforation by passing the ids got from just above string
...
if ($_GET["suc"]==1)
	$success = "The terms Record Has Been Successfully Added.";
if ($_GET["suc"]==2)
	$success = "The terms Record Has Been Successfully Updated.";
if ($_GET["suc"]==3)
	$success = "The terms Record Has Been Successfully Deleted.";	
?>