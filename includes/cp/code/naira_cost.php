<?php
	...
	$_sql = "SELECT `condition`,value FROM gcm WHERE condition_type='PER_LINE' and `condition`=1";
...
if($_SERVER['REQUEST_METHOD']=="POST" && $_POST['Input']=="Submit"){  
		$sql="UPDATE gcm 
				...
				where condition_type='PER_LINE' and `condition`=1";
			$isAllQueryExecuted= $_CONN->Execute($sql);
	if($isAllQueryExecuted){
		$success = "Naira Lotto Cost Updated Successfully.";
	}
