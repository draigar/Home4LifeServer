...
if (empty($_GET['nid'])) {
	if ($_CONN)
		  $_CONN->close();
	header("Location: ".$_DIR['site']['adminurl']."news".$atend."err".$_DELIM."17".$baratend);
} else { 
   $_sql = "select * from news where news_id=".$_GET['nid'];	
	...
	if (!$rs || $rs->EOF) {
		...
		if ($_CONN)
		  $_CONN->close();
		header("Location: ".$_DIR['site']['adminurl']."news".$atend."err".$_DELIM."1".$baratend);
	}
...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	if (empty($error)) {
		...
		if($isAllQueryExecuted){	
			header("Location: ".$_DIR['site']['adminurl']."news".$atend."suc".$_DELIM."2".$baratend);
			exit();
	   }
	} //if (empty($error))
