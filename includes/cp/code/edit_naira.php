...
if (empty($_GET['aid'])) {
	if ($_CONN)
		  $_CONN->close();
	header("Location: ".$_DIR['site']['adminurl']."current_naira".$atend."err".$_DELIM."51");
} else {
    $_sql="SELECT *,if(now()<from_time,0,1) as notedit FROM naira_lotto WHERE naira_id=".$_GET['aid'];
	...
	if (!$rs || $rs->EOF) {
		...
		if ($_CONN)
		  $_CONN->close();
		header("Location: ".$_DIR['site']['adminurl']."current_naira".$atend."err".$_DELIM."1".$baratend);
		exit();
	}
	elseif($_SERVER['REQUEST_METHOD'] != "POST") {

		if($rs->fields["notedit"]) {
			header("Location: ".$_DIR['site']['adminurl']."current_naira".$atend."err".$_DELIM."2".$baratend);
			exit();
		}
		...
		$_POST["cmbMonth"]        			= $from_time[1];
		$_POST["cmbYear"] 		  			= $from_time[0];
...
if($_SERVER['REQUEST_METHOD']=="POST" && $_POST['Input']=="Save"){
	...
	if(empty($error)) {
		...
		$_sql="select naira_id from naira_lotto where naira_id!=".$_GET['aid']." AND status='A' AND now()<=to_time AND
	...
	if(empty($error)){
		...
		if($isAllQueryExecuted){
			...
			header("Location: ".$_DIR['site']['adminurl']."current_naira".$atend."success".$_DELIM."2".$baratend);
			exit();
		}
	}
}
