...
if (empty($_GET['nid'])) {
	...
	header("Location: ".$_DIR['site']['adminurl']."faq".$atend."err".$_DELIM."17".$baratend);

} else { 

   $_sql = "select * from faq where faq_id=".$_GET['nid'];	
	...
	if (!$rs || $rs->EOF) {
		...
		header("Location: ".$_DIR['site']['adminurl']."faq".$atend."err".$_DELIM."1".$baratend);
	...
	if (empty($error)) {
		...
		{	
			...
			if($arr[0]>400) {
				...
				resize($newwidth,$newheight,$_DIR['inc']['faq_image'].$original);

			} else 

				copy($image,$_DIR['inc']['faq_image'].$original);

			chmod($_DIR['inc']['faq_image'],0755);
		...
		if($isAllQueryExecuted){	
			...
			header("Location: ".$_DIR['site']['adminurl']."faq".$atend."suc".$_DELIM."2".$baratend);
