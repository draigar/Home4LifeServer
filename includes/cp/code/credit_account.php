...
if($_SERVER['REQUEST_METHOD']=="POST" && $_POST['Input']=="Submit"){ 
	if (trim($_POST["txtAmount"])=="") {
		$_MSG[] = " Please enter amount to credit.";
		$error = 1;
	}	
	...
	if(empty($error)) {
		...
		$sql="insert into transaction values(NULL,'".$identifier."',now(),0,'".trim($_POST["txtAmount"])."','".trim($_POST["txtUserId"])."','C','C','".$_POST["txtDescription"]."','".$_SESSION['login']['userid']."',0,0,'D','".$_POST["cmbUserAgent"]."','".$balance."')";
		if($_CONN->Execute($sql)){
			$showDetail=false;
			send_email_credit_fund(trim($_POST["txtUserId"]),trim($_POST["txtAmount"]));
			$success = "The ".($_POST["cmbUserAgent"]=="U"?"User":"Agent")." account is successfully credited.";
			$_POST["txtUserId"]="";
			$_POST["txtAmount"]="";
