...
if(!empty($_GET['aid']) && $_GET['act']=="deact") //Delete Action
{
...
if(!empty($_GET['aid']) && $_GET['act']=="actv") //Delete Action
{
	//Include Get User  Query file
	 $_sql="UPDATE users 
		       SET status='A'
			   WHERE user_id=".$_GET['aid'];
	if ($rs=$_CONN->Execute($_sql)) { //User Not Exist
		$_MSG[] = "Activated Successfully";
		$rs->close();
...
//==============Code added by Jay=========================
	...
	$getAllIdsqry 			= $_DIR['admininc']['queries']."getadminuserids.sql";	//This will spcify the fiel which will give us only the ids of the country
	...
	$getDataByIdsqry 		= $_DIR['admininc']['queries']."getadminuserbyids.sql";	//This is the query file which will give us the country inforation by passing the ids got from just above string
