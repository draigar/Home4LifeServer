...
{
	$_sql = "SELECT news_id, image FROM news WHERE news_id=".$_GET['nid'];
	...
	if ($rs->EOF) {
		...
		$news_image = $rs->fields["image"];
	if ($rs)
		$rs->close();
	if (!$error) {
		if(!empty($news_image) && file_exists($_DIR['inc']['news_image'].$news_image))
		{
				unlink($_DIR['inc']['news_image'].$news_image);
				unlink($_DIR['inc']['news_image']."t".$news_image);
		}
		$_sql = "delete from news where news_id=".$_GET['nid'];
		$_CONN->Execute($_sql);
		header("Location: ".$_DIR['site']['adminurl']."news".$atend."suc".$_DELIM."3".$baratend);
		exit();
	}
}
if(empty($_POST['num']))
...
//==============Code added by Jay=========================
	...
	$getAllIdsqry 			= $_DIR['admininc']['queries']."getnewsids.sql";	//This will spcify the fiel which will give us only the ids of the country
	...
	$getDataByIdsqry 		= $_DIR['admininc']['queries']."getnewsbyids.sql";//This is the query file which will give us the country inforation by passing the ids got from just above string
...
if ($_GET["suc"]==1)
	$success = "The News Record Has Been Successfully Added.";
if ($_GET["suc"]==2)
	$success = "The News Record Has Been Successfully Updated.";
if ($_GET["suc"]==3)
	$success = "The News Record Has Been Successfully Deleted.";	
?>