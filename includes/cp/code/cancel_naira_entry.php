...
if($_SERVER['REQUEST_METHOD']=="POST" && $_POST['Input']=="Cancel") {	
	...
	if($count<=0){
	   ...
	   $error=1;	
		...
		for($i=0;$i<$count;$i++) {	
			$_sql = "UPDATE naira_entry_ticket SET cancel='Y' WHERE nt_id=".$_POST['chkAction'][$i];
...
if(!empty($_GET['id']) && $_GET['act']=="can"){
	$_sql = "UPDATE naira_entry_ticket SET cancel='Y' WHERE nt_id=".$_GET['id'];
	$rsSuccessful = $_CONN->Execute($_sql);
...
//==============Code added by Jay=========================
	...
	$getAllIdsqry 			= $_DIR['admininc']['queries']."getcancelnaira_entriesids.sql";	//This will spcify the fiel which will give us only the ids of the country
	...
	$getDataByIdsqry 		= $_DIR['admininc']['queries']."getcancelnaira_entriesbyids.sql";	//This is the query file which will give us the country inforation by passing the ids got from just above string
