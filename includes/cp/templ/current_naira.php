...
if(!empty($_GET['aid']) && $_GET['act']=="deact"){
	...
	$_sql = "SELECT naira_id FROM naira_lotto WHERE naira_id=".$_GET['aid'];
	...
	$_sql = "SELECT count(entry_id) as cnt FROM naira_entry WHERE naira_id=".$_GET['aid'];
	$rs = $_CONN->Execute($_sql);
	if ($rs && $rs->fields["cnt"]>0) { 
		$_MSG[] = "Cannot delete this game. Entries has been recorded, draw instead.";
		$error = 1;
	} 	
	if ($rs) $rs->close();
	
	if (!$error) { 
		$_sql="DELETE FROM naira_lotto WHERE naira_id=".$_GET['aid'];
		$_CONN->Execute($_sql);
		header("Location: ".$_DIR['site']['adminurl']."current_naira".$atend."success".$_DELIM."3".$baratend);
		exit(); 
	}
...
if(!empty($_GET['aid']) && $_GET['act']=="canc"){
	...
	if (!$error) { 
		$_sql="UPDATE naira_lotto SET status='C' WHERE naira_id=".$_GET['aid'];
		$_CONN->Execute($_sql);
		header("Location: ".$_DIR['site']['adminurl']."current_naira".$atend."success".$_DELIM."4".$baratend);
		exit(); 
	}
...
//==============Code added by Jay=========================
	...
	$getAllIdsqry 			= $_DIR['admininc']['queries']."getcurrent_nairaids.sql";	//This will spcify the fiel which will give us only the ids of the country
	...
	$getDataByIdsqry 		= $_DIR['admininc']['queries']."getcurrent_nairabyids.sql";	//This is the query file which will give us the country inforation by passing the ids got from just above string
...
if ($_GET["err"]==2) {
	$_MSG[] = "The Lotto game has started, You can't edit this game at this point.";
	$error  = 1;
}
if ($_GET["success"]==1)
	$success = "The Naira Lotto has been successfully Added.";
if ($_GET["success"]==2)
	$success = "The Naira Lotto has been successfully Updated.";
if ($_GET["success"]==3)
	$success = "The Naira Lotto has been successfully Deleted.";
if ($_GET["success"]==3)
	$success = "The Naira Lotto has been successfully Cancelled.";
