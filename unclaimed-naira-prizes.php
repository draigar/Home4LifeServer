...
$_THISPAGENAME='unclaimed-naira-prizes';

include_once($_DIR['inc']['code'].$_THISPAGENAME.'.php');

$content[0]="Unclaimed Naira Lotto Prizes";
...
<tr><td valign="top" background="<?=$_DIR["site"]["url"]?>images/img96.png" style="background-repeat-y">
	...
	<table width="98%" border="0" cellspacing="0" cellpadding="0" align="center">
	  ...
	  <tr> 
		...
		<td bgcolor="#009933" height="25px" class="text12" style="padding-left:10px"><font color="White">Largest 
	  ...
	  <tr> 
		...
		<td align="left" valign="top"> <table width="100%" border="0" cellspacing="0" cellpadding="4" align="center">
			...
			<tr> 
			  ...
			  <td height="37px" valign="top" class="text11">Vision Winning numbers</td>
			...
			<tr> 
			  ...
			  <td class="listing"><?=strlen($naira_id[$i])<4?substr("0000",0,4-strlen($naira_id[$i])).$naira_id[$i]:$naira_id[$i]?></td>

			  <td class="listing"><img src="<?=$_DIR["site"]["url"]?>images/naira.gif"> 
			  <?php 
				$totalamount=0;
				if($win_num[$i]) $totalamount=$match[$i][$matchp]/$prizesum[$naira_id[$i]][$matchp]; 
				if($visionwinner[$i]) $totalamount+=$visionwinneramount[$i];
				echo number_format($totalamount,2); 
			  ?> </td>

			  <td class="listing"> <?=$matchp==5? "Matched " .$matchp:"Jackpot"?></td>
