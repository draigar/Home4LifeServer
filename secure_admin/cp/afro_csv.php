...
$_PAY_METHOD="Payment Method";
        ...
        $sql="select fname,lname,date_entered,ticket_no,total_amount,date_format(claim_date,'%d-%m-%Y')as claim_date
			FROM afro_lotto l
			LEFT JOIN afro_entry e ON e.afro_id = l.afro_id
			LEFT JOIN afro_entry_ticket t ON t.entry_id = e.entry_id
			LEFT JOIN claimed c ON c.nt_id= t.nt_id
			LEFT JOIN users u ON u.user_id = e.user_id
			WHERE l.afro_id=".$_GET['id']."";
		...
		$out = '';
				...
				for ($i = 0; $i <8; $i++) {
				 ...
				 header("Content-Disposition: attachment; filename=afro_report.csv");
				 echo $out;	
				 exit();
}				 
?>
