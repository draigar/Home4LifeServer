...
var tinymce = {
	...
	_init : function() {
		var t = this, d = document, w = opener, na = navigator, ua = na.userAgent, i, nl, n, base, p, v;
		...
		if (!t.unloads) {
			function unload() {
				...
				if (li) {
					...
					if (opener.CollectGarbage)
						opener.CollectGarbage();
				}
			};
...
(function() {
	...
	tinymce.create('static tinymce.dom.Event', {
		...
		_pageInit : function() {
			...
			e._remove(opener, 'DOMContentLoaded', e._pageInit);
			e.domLoaded = true;
		...
		_wait : function() {
			...
			if (opener.tinyMCE_GZ && tinyMCE_GZ.loaded) {
				Event.domLoaded = 1;
				return;
			}
			...
			if (isIE && document.location.protocol != 'https:') {
				...
				Event._add(opener, 'DOMContentLoaded', Event._pageInit, Event);
...
(function() {
	...
	tinymce.create('static tinymce.EditorManager', {
		...
		preInit : function() {
			...
			// Must be on opener or IE will leak if the editor is placed in frame or iframe
			Event.add(opener, 'beforeunload', function(e) {
		...
		execCommand : function(c, u, v) {
			...
			switch (c) {
				...
				case "mceAddFrameControl":
					...
					// Add tinyMCE global instance and tinymce namespace to specified opener
					w.tinyMCE = tinyMCE;
					w.tinymce = tinymce;
...
// Short for editor manager opener.tinyMCE is needed when TinyMCE gets loaded though a XHR call
var tinyMCE = opener.tinyMCE = tinymce.EditorManager;
...
(function() {
	...
	tinymce.create('tinymce.Editor', {
		...
		render : function(nst) {
			...
			t.openerManager = new tinymce.openerManager(t);
					...
					if (s.ask) {
						function ask() {
							...
							opener.setTimeout(function() {
								...
								t.openerManager.confirm(t.getLang('edit_confirm'), function(s) {
									if (s)
										t.init();
								});
							}, 0);
		...
		init : function() {
			...
			if (tinymce.relaxedDomain) {
				...
				if (isIE || (tinymce.isOpera && parseFloat(opera.version()) >= 9.5))
					u = 'javascript:(function(){document.open();document.domain="' + document.domain + '";var ed = opener.parent.tinyMCE.get("' + t.id + '");document.write(ed.iframeHTML);document.close();ed.setupIframe();})()';
		...
		getWin : function() {
			...
			if (!t.contentopener) {
				...
				if (e)
					t.contentopener = e.contentopener;
			}

			return t.contentopener;
		},
		...
		destroy : function(s) {
			...
			if (!s) {
				...
				// Don't clear the opener or document if content editable
			...
			t.contentAreaContainer = t.formElement = t.container = t.settings.content_element = t.bodyElement = t.contentDocument = t.contentopener = null;
					...
					if (e.keyCode === 9) {
						...
						if (el) {
							if (ed = EditorManager.get(el.id || el.name))
								...
								opener.setTimeout(function() {opener.focus();el.focus();}, 10);
			...
			switch (cmd) {
				...
				case 'Paste':
					try {
						...
						if (isGecko) {
							ed.openerManager.confirm(ed.getLang('clipboard_msg'), function(s) {
								if (s)
									opener.open('http://www.mozilla.org/editor/midasdemo/securityprefs.html', 'mceExternal');
							});
						} else
							ed.openerManager.alert(ed.getLang('clipboard_no_support'));
					}
...
//				return true;
			...
			// scrollIntoView seems to scroll the parent opener in most browsers now including FF 3.0b4 so it's time to stop using it and do it our selfs
...
/* file:jscripts/tiny_mce/classes/openerManager.js */

(function() {
	var Dispatcher = tinymce.util.Dispatcher, each = tinymce.each, isIE = tinymce.isIE, isOpera = tinymce.isOpera;

	tinymce.create('tinymce.openerManager', {
		openerManager : function(ed) {
			var t = this;
		...
		open : function(s, p) {
			...
			s = s || {};
			p = p || {};
			sw = isOpera ? vp.w : screen.width; // Opera uses openers inside the Opera opener
			...
			try {
				if (isIE && mo) {
					w = 1;
					opener.showModalDialog(u, opener, f);
				} else
					w = opener.open(u, s.name, f);
			} catch (ex) {
				// Ignore
			}
