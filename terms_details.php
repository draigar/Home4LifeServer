...
if(empty($_GET['id'])){
	...
	$sql="select title,content,image,date_format(creation_date,'%D %M %Y')as creation_date from terms where terms_id=".$_GET['id'];
	...
	if($rs && !$rs->EOF){
		...
		$terms=stripslashes($rs->fields['content']);
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
	  ...
	  <td valign="top" class="text">Version:<em style="color:red;"><?php echo $creation_date; ?></em><div>

	  <?php if($image && file_exists($_SITE_ROOT_PATH."terms_image/".$image)) { 

	  	$arr=getimagesize($_SITE_ROOT_PATH."terms_image/".$image);

	  ?>

	  <img src="<?=$_DIR['site']['url']."terms_image/".$image?>" <?=$arr[0]>300?"width=\"300px\"":""?> style="float:left;margin-right:10px;margin-top:5px;" alt="<?=stripslashes($title)?>">

	  <?php } ?><?=stripslashes($terms)?></div></td>
